          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item: GalleryItem, index: number) => (
              <div
                ...
                className="group relative h-72 rounded-sm overflow-hidden border border-border shadow-card hover:shadow-card-hover hover:border-gold cursor-pointer transition-all duration-300 bg-navy-dark flex flex-col justify-end"
              >
                <img
                  src={item.image}
                  alt={item.title}
                  loading="lazy"
                  className="absolute inset-0 w-full h-full object-cover group-hover:scale-108 transition-transform duration-500 filter brightness-95"
                />
                ...
                <div className="absolute top-3 left-3 z-10">
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-sm bg-navy-dark/90 text-gold border border-gold/30">
                ...
                {/* Zoom Icon indicator */}
                <div className="absolute top-3 right-3 z-10 p-2 rounded-sm bg-navy-dark/80 text-white opacity-0 group-hover:opacity-100 group-hover:bg-gold group-hover:text-navy-dark transition-all">
                ...
                <div className="relative z-10 p-5 text-white">
                  {item.location && (
                    <div className="flex items-center gap-1 text-[11px] text-gold mb-1">
                      <MapPin className="w-3 h-3" />
                      <span>{item.location}</span>
                    </div>
                  )}
                  <h3 className="text-base font-bold uppercase tracking-wide group-hover:text-gold transition-colors line-clamp-1 font-heading">
                    {item.title}
                  </h3>
